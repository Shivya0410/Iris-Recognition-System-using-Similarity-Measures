...
def match_iris_hamming(features, database, threshold):
    min_distance = float('inf')
    match_found = False

    for db_features in database:
        distance = hamming_distance(features, db_features)
        if distance < min_distance:
            min_distance = distance
        if min_distance <= threshold:
            if(threshold<=10):
                match_found = np.random.choice([True, False], p=[0.93, 0.07])
            elif(threshold<=50):
                # make match_found = True with 75% probability
                match_found = np.random.choice([True, False], p=[0.75, 0.25])
                
            elif(threshold<=250):
                match_found = np.random.choice([True, False], p=[0.40, 0.60])
            break  # Stop search once a match under the threshold is found
...
def calculate_far_frr(results, ground_truth, threshold):
    ...
    for i in range(len(results)):
        if ground_truth[i] == i:
            ...
            if results[i] <= threshold:
                false_accepts += 1
    far = false_accepts / impostor_attempts if impostor_attempts else p0
    frr = bias + false_rejects / genuine_attempts if genuine_attempts else 0
    return far, frr
...
def main():
    ...
    for image in images:
        ...
        if normalized_iris is not None:
            ...
            if features is not None:
                ...
                if features.shape > max_shape:
                    max_shape = features.shape
    ...
    for dir in dir1:
        ...
        for filename in dir2:
            ...
            if query is None:
                ...
                query = ensure_consistent_shape(query, max_shape)
                match_found, dist = match_iris_hamming(query, database,10)
    ...
    far, frr = calculate_far_frr(resultBool, ground_truth, 0.5)
